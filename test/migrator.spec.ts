...
makeSuite("LEND migrator", (testEnv: TestEnv) => {
  ...
  it("Check the constructor is executed properly", async () => {
    ...
    expect(await lendToAaveMigrator.AAVE()).to.be.equal(
      aaveToken.address,
      "Invalid AAVE Address"
    );

    expect(await lendToAaveMigrator.LEND()).to.be.equal(
      lendToken.address,
      "Invalid LEND address"
    );

    expect(await lendToAaveMigrator.LEND_AAVE_RATIO()).to.be.equal(
      "1000",
      "Invalid ratio"
    );
  });
  ...
  it("Check migration isn't started", async () => {
    ...
    await expect(lendToAaveMigrator.migrateFromLEND("1000")).to.be.revertedWith("MIGRATION_NOT_STARTED");
  });
  ...
  it("Starts the migration", async () => {
    ...
    const migratorAsProxy = await getContract(
      ...
      await migratorAsProxy.connect(testEnv.users[0].signer).upgradeToAndCall(
  ...
  it("Migrates 1000 LEND", async () => {
    ...
    expect(aaveBalanceAfterMigration.toString()).to.be.eq(expectedAaveBalanceAfterMigration.toFixed(0));
  });
