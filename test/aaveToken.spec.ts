...
import { ProtocolErrors, eEthereumNetwork } from "../helpers/types";
...
makeSuite("AAVE token", (testEnv: TestEnv) => {
  ...
  it("Checks initial configuration", async () => {
    ...
    expect(await aaveToken.name()).to.be.equal(
      "Aave Token",
      "Invalid token name"
    );

    expect(await aaveToken.symbol()).to.be.equal(
      "AAVE",
      "Invalid token symbol"
    );

    expect((await aaveToken.decimals()).toString()).to.be.equal(
      "18",
  ...
  it("Checks the allocation of the initial AAVE supply", async () => {
    const expectedMigratorBalance = new BigNumber(13000000).times(
      new BigNumber(10).pow(18)
    );
    const expectedlDistributorBalance = new BigNumber(3000000).times(
      new BigNumber(10).pow(18)
    );
    const { aaveToken, lendToAaveMigrator } = testEnv;
    const migratorBalance = await aaveToken.balanceOf(
      lendToAaveMigrator.address
    );
    const distributorBalance = await aaveToken.balanceOf(
  ...
  it("Starts the migration", async () => {
    ...
    const lendToAaveMigratorInitializeEncoded = lendToAaveMigratorImpl.interface.functions.initialize.encode(
      []
    );
  ...
  it("Checks the snapshots emitted after the initial allocation", async () => {
    ...
    const userCountOfSnapshots = await aaveToken._countsSnapshots(
      users[0].address
    );
    const snapshot = await aaveToken._snapshots(
      users[0].address,
  ...
  it("Record correctly snapshot on migration", async () => {
    ...
    await waitForTx(
      await lendToken.approve(
        lendToAaveMigrator.address,
        ethers.utils.parseEther("2000")
      )
    );
    await waitForTx(
      await lendToAaveMigrator.migrateFromLEND(ethers.utils.parseEther("2000"))
    );
  ...
  it("Record correctly snapshot on transfer", async () => {
    ...
    const fromLastSnapshot = await aaveToken._snapshots(
      from,
      fromCountOfSnapshots.sub(1)
    );
    const fromPreviousSnapshot = await aaveToken._snapshots(
      from,
      fromCountOfSnapshots.sub(2)
    );

    const toCountOfSnapshots = await aaveToken._countsSnapshots(to);
    const toSnapshot = await aaveToken._snapshots(
      to,
  ...
  it("Reverts submitting a permit with 0 expiration", async () => {
    ...
    const { chainId } = await BRE.ethers.provider.getNetwork()
    if (!chainId) {
  ...
  it("Submits a permit with maximum expiration length", async () => {
    ...
    const { chainId } = await BRE.ethers.provider.getNetwork()
    const configChainId = BRE.network.config.chainId;
  ...
  it("Correct snapshotting on double action in the same block", async () => {
    ...
    const countSnapshotsReceiver = (
      await aaveToken._countsSnapshots(receiver)
    ).toString();

    const snapshotReceiver = await aaveToken._snapshots(
      doubleTransferHelper.address,
      0
    );
  ...
  it("Emits correctly mock event of the _beforeTokenTransfer hook", async () => {
    ...
    await expect(
      aaveToken.connect(users[1].signer).transfer(recipient, 1)
