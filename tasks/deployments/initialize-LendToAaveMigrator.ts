...
task(
  `initialize-${LendToAaveMigrator}`,
  `Initialize the ${LendToAaveMigrator} proxy contract`
)
  .addParam(
    "admin",
    "The address to be added as an Admin role in Aave Token Transparent Proxy."
  )
  .addFlag(
    "onlyProxy",
    "Initialize only the proxy contract, not the implementation contract"
  )
  .setAction(async ({ admin: aaveAdmin, onlyProxy }, localBRE) => {
    ...
    console.log(
      "\tFinished LendToAaveMigrator Proxy and Implementation initialization."
    );
  });
