...
task(`initialize-${AaveToken}`, `Initialize the ${AaveToken} proxy contract`)
  .addParam(
    "admin",
    `The address to be added as an Admin role in ${AaveToken} Transparent Proxy.`
  )
  .addFlag(
    "onlyProxy",
  ...
  .setAction(async ({ admin: aaveAdmin, onlyProxy }, localBRE) => {
    ...
    if (onlyProxy) {
      ...
      await waitForTx(
        await aaveTokenProxy.initialize(aaveTokenImpl.address, aaveAdmin, "0x")
      );
      console.log(
    ...
    const aaveTokenEncodedInitialize = aaveTokenImpl.interface.functions.initialize.encode(
      [lendToAaveMigratorProxy.address, aaveAdmin, ZERO_ADDRESS]
    );
